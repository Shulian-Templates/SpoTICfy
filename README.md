...
- Crear la base de datos `spoticfy` en `Neon` e importar el archivo `spoticfy.sql` que se encuentra en la raíz del proyecto.
...
Para cada endpoint, debe haber **al menos** un test de la ruta en Isnomnia. Cuando terminen todos los endpoints, exportar la colección de tests en `pruebas`.

## Entrega y defensa

Para entregar alcanza con pushear el repositorio. Luego se va a tomar un coloquio a cada uno de los integrantes donde se espera que sepan defender el trabajo realizado.